...
namespace Discord.Interactions
{
    ...
    public class ComponentCommandInfo : CommandInfo<ComponentCommandParameterInfo>
    {
        ...
        {
            if (paramList?.Count() < wildcardCaptures?.Count() + 1)
            {
                var result = ExecuteResult.FromError(InteractionCommandError.BadArgs, "Command was invoked with too many parameters");
