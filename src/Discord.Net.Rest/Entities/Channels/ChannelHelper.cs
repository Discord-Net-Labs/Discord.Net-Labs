...
namespace Discord.Rest
{
    internal static class ChannelHelper
    {
        ...
        {
            var listEmbeds = new List<Embed>();

            if (embed != null)
                listEmbeds.Add(embed);

            if (embeds != null)
                listEmbeds.AddRange(embeds);

            Preconditions.AtMost(allowedMentions?.RoleIds?.Count ?? 0, 100, nameof(allowedMentions.RoleIds), "A max of 100 role Ids are allowed.");
            Preconditions.AtMost(allowedMentions?.UserIds?.Count ?? 0, 100, nameof(allowedMentions.UserIds), "A max of 100 user Ids are allowed.");
            Preconditions.AtMost(listEmbeds.Count, 10, nameof(embeds), "A max of 10 embeds are allowed.");
            ...
            {
                IsTTS = isTTS,
                Embeds = listEmbeds.Any() ? listEmbeds.Select(x => x.ToModel()).ToArray() : Optional<API.Embed[]>.Unspecified,
